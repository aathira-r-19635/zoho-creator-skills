...
| Create Form Schema + Seed via CSV Import | `creator-csv-import-schema.md` | Stand up Creator form schema + seed data fast via CSV "Import with data" (auto-fields from headers; choice fields get allow_other_choice); ID precision-loss gotcha + fix (verified 2026-06-14) |
| Seed Multiple Related Entities via CSV | `creator-multi-entity-seeding.md` | Seed 3+ interdependent Creator forms via parallel CSV imports; dependency order, cross-reference validation, widget hydration verification (verified 2026-06-14) |
